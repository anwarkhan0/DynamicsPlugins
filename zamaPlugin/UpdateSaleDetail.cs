...
{
    public class UpdateSaleDetail : IPlugin
    {
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
                ...
                context.InputParameters["Target"] is Entity)
            {
                ...
                try
                {
                    Entity PreSaleDetails = (Entity)context.PreEntityImages["PreSale"];
                    decimal unitPrice = saleDetail.Attributes.Contains("cr651_unitprice") ? ((Money)saleDetail.Attributes["cr651_unitprice"]).Value : ((Money)PreSaleDetails.Attributes["cr651_unitprice"]).Value;
                    int quantity = saleDetail.Attributes.Contains("cr651_quantity") ? (int)saleDetail["cr651_quantity"] : (int)PreSaleDetails["cr651_quantity"];
                    decimal x = unitPrice * quantity;
                    decimal totalAmount = x;
